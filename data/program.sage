...
if d == 5:
	...
	print "with independence number at most 3, and without K_7 or K_{3,3,3}."
	forbidden_subgraph = k3_3_3 = graphs.CompleteMultipartiteGraph([3,3,3])

if d == 6:
	print "A five-dimensional abstract almost-equidistant graph is a graph"
	print "with independence number at most 3, and without K_8 or K_{1,3,3,3}."
	forbidden_subgraph = k1_3_3_3 = graphs.CompleteMultipartiteGraph([1,3,3,3])
...
while True:
	...
	if enumerate_graphs:
		filename = "graphs"+str(d)+"_"+str(i)+(".g6" if encode else ".txt")
		with open(filename,"w") as f:
			for g in gr[i]:
				s = g.graph6_string() if encode else str((g.vertices(),g.edges(labels=False)))
				f.write(s+"\n")
		print "graphs written to file:",filename